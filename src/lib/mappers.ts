...
export function toDiligencia(row: DiligenciaRow): Diligencia {
  return {
    ...
    observacoes: row.observacoes ?? undefined,
    obsAdvogado: row.obs_advogado ?? undefined,
    ...
    pesquisa: {
      ...
      tentativasWhatsApp: row.pesquisa_tentativas_whatsapp ?? 0,
      dataConclusao: row.pesquisa_data_conclusao ?? undefined,
    },
    anexos: {
    ...
    dataEvento: row.data_evento ?? undefined,
...
export function fromDiligencia(
  ...
  return {
    ...
    ...(d.dataEvento != null && { data_evento: d.dataEvento }),
